"""Validate LLM extraction results for completeness and quality."""
import re
...
def validate_extraction(
    ...
    Args:
        paragraphs: List of paragraph dicts from the extractor.
    ...
    numbers = [p["section_number"] for p in paragraphs]
    seen = set()
    for n in numbers:
        if n in seen:
    ...
    for p in paragraphs:
        if not p.get("content", "").strip():
            report.add(f"Empty content for § {p.get('section_number', '?')}")

    # Check for gaps in sequential numbering
    numeric_values = []
    for p in paragraphs:
        val = _extract_numeric(p["section_number"])
        if val is not None:
